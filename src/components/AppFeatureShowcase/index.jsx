...
const AppFeaturesShowcase = () => {
  ...
  const features = [
    ...
    },
    {
      ...
      description: "Your account is your gateway to enriching your understanding of Islam. Generte an account now and get solution of your questions",
  ...
  return (
    <div className={tw(`py-20 bg-gray-50`)}>
      <div className={tw(`max-w-7xl mx-auto px-4 sm:px-6 lg:px-8`)}>
        <div className={tw(`flex flex-col lg:flex-row gap-12`)}>
            ...
            <ul className={tw(`space-y-8`)}>
              {features.map((feature) => (
                <li
                  key={feature.id}
                  className={tw(
                    `flex items-start cursor-pointer border-b pb-6 ${activeFeature === feature.id ? "border-teal-500" : "border-gray-200"}`,
                  )}
                ...
                >
                  ...
                  <div>
                    ...
                    {activeFeature === feature.id && feature.description && (
                      <p className={tw(`mt-2 text-gray-600`)}>{feature.description}</p>
                    )}
                  </div>
                </li>
              ))}
                  ...
                  {feature.images.map((image, index) => {
                    ...
                    return (
                      <div
                        key={index}
                        className={tw(
                          `absolute ${positions[index] || positions[0]} w-[200px] md:w-[220px] transform transition-all duration-500`,
                        )}
                      >
                        <img
